...
//                Logger.error("ERROR InterpolatedMap, empty map used");
                return 0;
            } else if (floorKey == null) {
                Map.Entry<Double, Double> result = super.firstEntry();
//                Logger.error("ERROR InterpolatedMap key: " + key + " lower than floor: " + result.getKey());
                return result.getValue();
            } else if (ceilKey == null) {
                Map.Entry<Double, Double> result = super.lastEntry();
//                Logger.error("ERROR InterpolatedMap key: " + key + " higher than ceil: " + result.getKey());
                return result.getValue();                
            }  
